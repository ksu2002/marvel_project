...
class FavoriteFragment : Fragment() {
    ...
    private val characterCLickListener by lazy {
        CharactersAdapter.CharacterCLickListener { id ->
            ...
            transaction?.add(R.id.fragment_container_view, fragment, "fragmentCharacter")?.hide(
                active
    ...
    private fun setupObservers() {
        viewModel.getFavorites()?.observe(viewLifecycleOwner) { favorite ->
            favorite ?: return@observe
            mainAdapter.addCharacters(favorite)
        }
    }
}